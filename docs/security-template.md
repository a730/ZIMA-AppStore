# Security Guide for App Name

> Copy this file to `Apps/your-app/SECURITY.md` and fill in the details.
...
| Database password | Set via `DB_PASS` env var | Recommended |
...
|------|----------|------------|------------------|
| User content | `/data` volume | None at rest | Yes |
| Configuration | `/config` volume | None | Yes |

## Network Access

- The app connects to the internet for [update checks / model downloads / API calls].
...
tar -czf backup-$(date +%Y%m%d).tar.gz /DATA/AppData/your-app/
...
2. Do not expose database ports (5432, 6379, 3306) to the host LAN
